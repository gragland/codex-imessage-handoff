...
5. Respond with `localMessage` exactly and nothing else. Do not include debug details unless the user explicitly asks for them.
   ...
   A paired phone can have multiple active remote threads. Starting remote for this thread switches iMessage to this thread. The user can text `list` or `threads` to the Sendblue number to see numbered active threads, then text a bare number such as `2` to switch.
...
Text `list` to the Sendblue number to see all active remote threads for the paired phone. Text a number from that list to switch which thread receives normal remote messages.
